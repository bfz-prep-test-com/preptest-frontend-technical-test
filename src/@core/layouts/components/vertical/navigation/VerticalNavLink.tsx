...
import { getSelectorFromNavigationName } from 'src/static/data/tour/dashboard'
...
const VerticalNavLink = ({
  ...
  return (
    <CanViewNavLink navLink={item}>
      <ListItem
        id={getSelectorFromNavigationName(item.title).toString()}
        sx={{
          transition: 'padding .25s ease-in-out',
          px: parent ? '0 !important' : `${theme.spacing(navCollapsed && !navHover ? 2 : 3)} !important`
        }}
      >
        <Link style={{ width: '100%' }} href={item.path === undefined ? '/' : `${item.path}`}>
          <a style={{ width: '100%' }} href={item.path === undefined ? '/' : `${item.path}`}>
            <MenuNavLink
              ...
              sx={{
                ...
                pr: navCollapsed && !navHover ? (collapsedNavWidth - navigationBorderWidth - 24 - 16) / 12 : 2,
                pl: navCollapsed && !navHover ? (collapsedNavWidth - navigationBorderWidth - 24 - 16) / 12 : 2
              }}
            >
              {isSubToSub ? null : (
                <ListItemIcon
                  sx={{
                    ...
                    ...(parent ? { ml: 2, mr: 3 } : {}) // This line should be after (navCollapsed && !navHover) condition for proper styling
                  }}
                >
                  <UserIcon
                    icon={IconTag}
