...
const VerticalNavHeader = (props: Props) => {
  ...
  const menuHeaderPaddingLeft = () => {
    if (navCollapsed && !navHover) {
      if (userVerticalNavMenuBranding) {
        ...
        return (collapsedNavWidth - navigationBorderWidth - 40) / 10
      }
    } else {
      return 3
    }
  ...
  const svgRotationDeg = () => {
    if (navCollapsed && direction === 'rtl' && navHover) return 0
    if (navCollapsed && direction === 'rtl' && !navHover) return 180
    if (navCollapsed && direction !== 'rtl' && navHover) return 180
    if (navCollapsed && direction !== 'rtl' && !navHover) return 0
    if (!navCollapsed && direction === 'rtl') return 180
  ...
  return (
    <MenuHeaderWrapper className='nav-header' sx={{ pl: menuHeaderPaddingLeft() }}>
      {userVerticalNavMenuBranding ? (
        ...
        <Link href='/'>
          <StyledLink href='/'>
            <CustomImage height={32} width={30} src='/logo.svg' alt='logo' />
            <HeaderTitle
              variant='body1'
      ...
      {hidden ? (
        <IconButton
          ...
          sx={{
            ...
            color: theme => (theme.palette.mode === 'dark' || skin === 'semi-dark' ? 'white' : 'black')
          }}
        >
